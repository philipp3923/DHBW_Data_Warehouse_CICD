...
conn = psycopg2.connect(
    dbname="postgres", user="postgres", password=os.environ["POSTGRES_PASSWORD"]
)
cur = conn.cursor()
...
def on_message(client, userdata, msg):
    ...
    cur.execute(
        "insert into staging.messung (payload) values (%s)", (msg.payload.decode(),)
    )
    conn.commit()
...
client = mqtt.Client(
    ...
    "piuabsdfuwaeg4wq98thqb3p4bg43gq8b9ur0vb97",
    clean_session=False,
)
